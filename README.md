...
- **打包下载**：
  - 点击"打包下载 ZIP"按钮直接下载处理后内容
  - 点击下拉箭头选择下载模式：原始内容 / 处理后内容 / 同时下载两种内容
...
#### 新增功能
- **批量下载 ZIP 功能增强**：支持三种下载模式
  - 下载原始内容：下载编辑框中的当前内容（可能已被用户修改）
  - 下载处理后内容：下载加密/解密后的结果（默认行为）
  - 同时下载两种内容：自动分文件夹存储（original/ 和 processed/）
- 使用分裂按钮设计：主按钮直接下载，下拉箭头展开更多选项

#### 技术改进
- 新增 `DownloadMode` 类型定义（'original' | 'processed' | 'both'）
- 扩展 `downloadAsZip` 函数支持多种下载模式
- 更新单元测试覆盖新功能
